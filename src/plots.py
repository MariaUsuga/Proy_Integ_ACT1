...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    sns.scatterplot(data=df, x="total_product_weight_g", y="total_freight_value")
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    for index, row in df[df["holiday"] == True].iterrows():
        plt.axvline(x=row["date"], color="red", linestyle="--")  # Completa la línea faltante
    plt.xlabel("Date")
